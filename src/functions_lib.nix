{ nixpkgs }:
let
  cp = "${nixpkgs.coreutils}/bin/cp";
  echo = "${nixpkgs.coreutils}/bin/echo";
  sleep = "${nixpkgs.coreutils}/bin/sleep";
  sort = "${nixpkgs.coreutils}/bin/sort";
  uniq = "${nixpkgs.coreutils}/bin/uniq";
  wc = "${nixpkgs.coreutils}/bin/wc";

  inherit (nixpkgs.lib) getExe;
  grep = getExe nixpkgs.gnugrep;
  jq = getExe nixpkgs.jq;
  rofi = getExe nixpkgs.rofi;
